...
**A4** The population of Kanawha is significantly higher than the population of Wirt. This could mean that there are more businesses that need funding in Kanawha due to the higher population.
...
**A5** The Greenbriar Hotel Corporation got the state's largest PPP loan for 8,912,202 dollars.The business is a 5-star hotel and the loan retained 500 jobs. 
...
**A6** Jim Justice, the governor of West Virginia, owns the Greenbriar Hotel Corporation. My source of this information is the Office of the Governor. (https://governor.wv.gov/Pages/default.aspx)
...
**A7** Something that stands up to me that I could imagine dropping into a news story is the minimum loan amount versus the maximum loan amount. The range of these two pieces of data is so vast that it would make for a very interesting comparison. 
```{r}
wv_ppp_loans %>%
  summarise(
    count_loans = n(),
...
**A8** Something that stands out to me that I could imagine dropping into a news story is the different amounts of loans that each county was given. There is a huge difference between the amounts of loans given to each county. For example, Tyler County Versus Webster County recieved a very different amount of loans. 
```{r}
  wv_ppp_loans %>%
  group_by(project_county_name) %>%
  summarise(
