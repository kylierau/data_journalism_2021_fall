...
```{r}
maryland_ppp_naics_codes <- maryland_ppp %>% left_join(naics_codes) %>%
...
```{r}
maryland_ppp_naics_codes_zcta <- maryland_ppp_naics_codes %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
```

## Answer questions

**Q1** What are the top 5 industries (using the NAICS title) with the most PPP applications in Maryland?
**A1** Taxi Service, Beauty Salons, Full-Service Restaurants, Officers of Real Estate Agents and Brokers and All Other Personal Services are the top 5 industries with the most PPP applications in Maryland. 

```{r}
maryland_ppp_naics_codes_zcta %>%
  group_by(title) %>%
  summarize(NAICS = n()) %>%
  arrange(desc(NAICS))
```
...
**A2** Baywood Hotels Inc., Partners Electric Service Inc, RP3 LLC, Sushi Ko Chevy Chase LLC. and Village Academy of Maryland are the top industries with the most PPP applications in Maryland where the loan amount is more than 150,000 dollars. 
...
**A3** 20850 had the largest sum of PPP loans of any Maryland zip code with at least 20 percent non-Hispanic Asian population. 
...
**A4** 20850 had the largest sum of loans where the reported race of the applicant was "Asian". The percentage of non-Hispanic Asian population in that zip code was 23.5 percent.  
...
**A5** Although 20850 has a significant Asian population, not all loans were distributed to Asian applicants. This is due to the racial disparities that existed within the distribution of PPP loans. 
...
**A6** There are 3 zip codes that have loans where the PPP data lists the applicants as being Asian but where the zip code does not appear to have Asian population in the ZCTA data. This could be the result of business owners not living in the zip code of where they own a business. 
...
**A7** There are 44 industries that have exactly one loan in the data. I think the ammunition and casinos are most worthy of further exploration because these are large institutions.
