...
**A1.** Children between 5-11 years old have recorded a daily change in first daily dose of -49.989% and a weekly change of -70.75%, both negative changes that increased in the negative direction over time. 
...
**A2.** The non-Maryland top three zip codes are 685, 716 767. These are zip codes in Puerto Rico. The top 3 Maryland zip codes are 20906, 20878, 20874 (20878 is actually my home zip code in Gaithersburg!). All of these zip codes are Montgomery County locations. 
...
non_maryland_zip <- county_vaccinations_by_zip %>%
  filter(zip_code < 20588 | zip_code > 21931) %>%
  filter(zip_code != 0)
 
 
top_25_maryland <- maryland_zip %>%
 head(25)

top_25_non_maryland <- non_maryland_zip %>%
  head(25)

  
write.csv(top_25_maryland, paste0("data/maryland_zips_yyyymmdd.csv" , today(), ".csv"))

write.csv(top_25_non_maryland , paste0("data/non_maryland_zips_yyyymmdd.csv" , today(), ".csv"))
```
