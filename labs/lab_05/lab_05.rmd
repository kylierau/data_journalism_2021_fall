...
**A1.** The street address in this data that has the most loans is 126 E. Burke St. There are 6 loans at that street address. The businesses at this address are ABRAHAM ASHTON, BRIX27, LLC, DREW HOLDINGS LLC, HUB CO-OP LLC(2 loans) and RONIN PROPERTIES LLC. 

```{r}
zip_25401_loan_sample <- read_csv("data/zip_25401_loan_sample-csv.csv") %>%
  group_by(address) %>%
  summarise(
    count=n()
  )
...
**A2.** This collection of businesses all seem very suspicious as they seem as though they are all owned and run by someone by the name of Abraham Ashton. Ashton has his own law firm, which is receiving loans, and is somehow involved with all of the different companies at this address that are receiving loans. One that stands out is the Hub Co-op, which is a community work space located at 126 E. Burke Street. This could be suspicious because on the website there are no other businesses listed as having set space in The Hub. As far as the website shows, this location is it's own individual business in which people can pay to use work space. Overall, this is suspicious.
...
**A3.** Martinsburg had the highest number of loans in Berkeley. Full service restaurants had more loans than any other industry title. 

```{r}
lab_05 %>%
  filter(project_county_name == "BERKELEY") %>%
write.csv("data/berkeley_loans.csv", row.names = TRUE)

berkeley_loans <- read_csv("data/berkeley_loans-csv.csv")
...
cleaned_berkeley <- read_csv("data/berkeley_loans-csv.csv")

berkeley_naics <- cleaned_berkeley %>% 
...
**A4.** One hypothesis of why full service restaurants are at the top of the loan list is because of the pandemic, these restaurants had the most trouble making money due to government shut downs. Another hypothesis is that Martinsburg has a lot of full-service restaurants. After further research I found that Martinsburg has a plethora of different full-service restaurants.
...
**A5.** LAST RESORT LOUNGE LLC was the business that got the highest approved loan amount. It was for 46795.00. It was approved on February 24, 2021. There were 19 jobs retained. The customers know this business as VIP Lounge. This business has two locations. There was a third location that closed during the pandemic. 
```{r}
berkeley_naics %>%
  filter(city == "martinsburg" ) %>%
