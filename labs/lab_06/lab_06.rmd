...
```{r}
international_migration <- read_csv("data/tabula-md-statistical-handbook.csv", skip=2, col_names=c("Name", "July_2010", "July_2011", "July_2012", "July_2013", "July_2014", "July_2015", "July_2016", "July_2017", "July_2018", "Total"))
```
...
**A1** The Suburban Washington Region has the largest percentage of international migration for the entire time period (63%). Montgomery County is hte count that accounted for the largest percentage of international migration for the entire time period (37%). 

```{r}
international_migration <- international_migration %>% mutate(percent_of_total=Total / 198996)

```

**Q2** Write a sentence or two that describes the data you produced in A1. Try to convey the most important idea.
**A2** The data I produced in A1 is a column for percent of total that calculates the region/county/city percentages of international migration. I divided each total by the entire state of Maryland total to calculate this average in my code.
...
**A3** Cecil County had the largest percentage change (negative) for international migration between July 2018 and July 2017 (-127%). 

```{r}
international_migration <- international_migration %>% mutate(percentage_change =(July_2018-July_2017) / July_2017)
```

**Q4** What's your best guess as to why these changes occurred, and in those places in particular?
**A4** My best guess as to why these changes occurred in certain places is that there have been significant changes in legislation against immigration nationally (Donald).
...
```{r}
domestic_migration <- read_csv("data/tabula-md-statistical-handbook-domestic.csv", skip=2, col_names=c("Name", "July_2010", "July_2011", "July_2012", "July_2013", "July_2014", "July_2015", "July_2016", "July_2017", "July_2018", "Total"))
```
```
...
**A5** Baltimore City saw the largest net decrease in domestic migration overall (-62834).
...
**A6** 2 Regions and 12 jurisdiction had net positive migration for July 2018, July 2018 and July 2019. 
...
**A7** This data shows a wide variety of change over time in migration, representing the complexity of different regional data. Howard County, Charles County and Worcester County stood out to me, as they had net positive migration the entire time. 