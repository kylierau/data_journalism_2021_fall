...
A1. The U.S. Department of Justice filed 89 cases against people accused of fraud related to the PP or EIDL loan program. 

```{r}
url <- "https://www.justice.gov/criminal-fraud/cares-act-fraud"

individual_cases <- url %>%
  read_html() %>%
  html_elements("b") %>%
  html_text() %>%
  as_tibble() %>%
...
A2. The U.S. Department of Justice filed cases against people accures of fraud related to the PP or EIDL loan program in 28 individual judicial districts.

```{r}
url <- "https://www.justice.gov/criminal-fraud/cares-act-fraud"

individual_districts <- url %>%
  read_html() %>%
  html_elements("b") %>%
  html_text() %>%
  as_tibble() %>%
...
A4. Diamond Blue Smith is a recording artist. He was accused of participating in a scheme to file fraudulent loan applications seeking more than $24 million in forgivable PPP loans. HE was charged in federal criminal complaints filed in the Southern District of Florida with wire fraud, bank fraud and conspiracy to commit wire fraud and bank fraud.  If I were an editor, I would have assigned a reporter to write a story about this case because of the different loans that Smith obtained. It would have been interesting to pull data from the SBA to determine exactly how Smith obtained each of these loans. There were a lot of others involved in this case, so it would be a larger story than just Smith committing fraud. 
...
A5. The U.S. Department of Justice has filed cases against people in 29.79% of all judicial districts.
...
A6. One reason why the DOJ could have filed PPP-related fraud cases in less than half of the country's judicial districts is population distribution. Another reason that the DOJ might have filed PPP-related fraud cases in less than half of the country's judicial districts is due to the different levels of courts.
...
A7.
```{r}
url <- 'https://smussenden.github.io/coursefiles/ppp_scraping_example/index.html'
...
print(52/nrow(state_url))


PPP_loan_totals <- url %>%
  read_html() %>%
  html_table()%>%
  state_page_link <- 'state_page_link' %>%
  for (x in state_page_link) {
  print(x) }
...
PPP_loan_totals <- url %>%
  read_html() %>%
  html_elements(xpath_employment_table) %>%
  html_table()

print(PPP_loan_totals)
