...
ppp <- read_csv("data/state_ppp_all.csv")
...
scale_x_date(date_breaks = "1 day", date_labels = "%d%b-%Y") +
 theme(
  ...
  geom_text(aes(label=total_loans), nudge_x=-1, nudge_y=-1, check_overlap = T)

```
**A1.** April 28 is the day in 2020 that had the most loans approved in West Virginia.
...
sf1 <- load_variables(2019, "acs5", cache = FALSE)

ACS2019 <- get_acs(geography = "state",
             variables = c(label="B01001_001"),
              year = 2019)
...
state <- other_acs %>%
left_join(ppp, by=c("state")) %>%
mutate(change = (total_ppp_loans/estimate)*100000) %>%
  arrange(desc(change)) %>%
  filter(change > 4388.588)


state %>%
  ggplot() +
  geom_bar(aes(x=reorder(state,total_ppp_loans)), weight=total_ppp_loans, fill="#0099f")
...
WV %>%
  ...
  mutate(forgiveness_amount = case_when(is_no(forgiveness_amount)-0, 
   TRUE = forgiveness_amount
  ))
  WV %>%
  ggplot() + 
  geom_point(desc(c=amount,y=forgiveness_amount))
