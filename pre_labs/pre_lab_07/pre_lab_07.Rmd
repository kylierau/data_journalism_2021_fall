...
**Answer** This web page has multiple different tables of data. The first table is NAICS Conversion Implementation Dates for the CES Program, Table 2 has NAICS Sectors with their proper descriptions, Table 3 has SIC Divisions with their descriptions and the fourth table compares the coding structure of SIC versus NAICS. 
The second table is the most significant to what we are analyzing here, as it shows us NAICS codes about specific industries. This will allow us to figure out whwat top-level industries got loans. 
...
**Answer** When I click "view source" I see the code used for the creation of the webpage. This site uses HTM as well as Javascript. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the code block.
**Answer** The output introduces the HTMl document, and the language as well as sets up the header and body layout. This appears in the environment window. 
...
**Answer** The naics_industry data pulls up a oage with three columns: name, type and value. html, head and body are all lists with two values. There is also row for xml attributes which are the characters that establish the language as "en-us". 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** I see a new window with code that looks simliar to R studio or Atom. Highlighted is the line <td class="line-content">.
...
**Answer** Multiple different outputs appear below the codeblock. Each of these windows contain data from a different table from the website.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The new naics_industry is now a list of 6, which has 6 different rows representing each one of the tables with data about the amount of rows and columns on each one. 
...
**Answer** When I click the scroll icon for the first dataframe it opens up a new window with a table that has conversion, reference month implemented and date released as the column titles. 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** This table shows the NAICS code with the proper industry description.  
...
**Answer** THe output that appears below the codeblock is a table with the sector number as well as the proper industry description. This table also has a section for the source.  
...
**Answer** This cleaned out the data by removing the source row and standardizing column headers. This made the data easier to navigate and more organized. 
...
**Answer** The output that appears below the codeblock is the data table with the sector numbers as well as the description of the industry. This is the table that we created in the last section. 
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This web page has data about labor industries including employement and unemployment statistics, union membership and representation statistics, dynamic changes in employment, employment by occupation, employee earnings and benefits tables as well as data about workforce fatalities, injuries and illnesses.
...
**Answer** The output starts off the html document and establishes the language, as well as creates a header and body section for the data. 
...
**Answer** The output shows the code for creating a table. This singles out the table with employment information and displays it. 
...
**Answer** The output is a table which is pretty unorganized. It shows different categories and dates with numbers about employment and unemployment information. 
...
**Answer** This output cleaned the table into a singular row with employment data for june of 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This webpage had information about the Mininig, Quarrying and Oil and Gas Extraction sector. This data included workforce statistics; earnings and hours; fatalities, injuries and illnesses; prices and workplace trends.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This web page had specific information about the construction sector. 
...
**Answer** The output is broken into sections by different industries. 
```{r}
...
**Answer** No output appears below the codeblock when I run this code. 
```{r}
...
**Answer** This output prints out the list of sectors and it displays the same way as the print functions displayed using less code. 
```{r}
...
**Answer** Thie output that appears says 'error in sector_list : object 'sector_list' not found'. This means that there is nothing titles sectors list. 
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** This output prints a list of sectors with x as the defining character for dispalying this list of sectors . 
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** This code displays an error that reads 'error in print(sector_name) : object 'sector_name' not found'. This is because we did not define "sector_name" anywhere. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** The output is a list from 1-10 with [1] in front of each number. 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** This code displays sector numbers along with their corresponding industry. 
...
**Answer** This code displays the number of rows in naics_industry, 20. 
...
**Answer** This code outputs the numbered list from 1-20, each row in naics_industry. 
...
**Answer** This is the table that has a row of employment data from June 2021. 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** This is the table with the sector and the corresponding industry. 
```{r}
# Define url of page we want to scrape
...
**Answer** The output shows the sector numbers with their corresponding industry as well as their corresponding URL.
...
**Answer** This output is the same as above but with a column called "sector_xpath_id" that is the corresponding ID for the table that we want information from. 
...
**Answer** This output removed the "Public Administration" sector from the data. 
...
**Answer** The output divides different sectors into their own singular row tables. 
...
**Answer** This output displays code that is a loop of row with their url and corresponding xpath value. 
```{r}
...
**Answer** The output below this displays very unorganized tables with data about industries by date. 
...
**Answer** This output is different tables for each individual data series (employment etc). 
...
**Answer** This code creates an output that adds in the data to the tables
...
**Answer** This code displays a table with three columns, sector numbers, the corresponding industry and employee information for June 2021. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** This web page displays data about the agriculture, forestry, fishing and hunting sector. 
...
**Answer** This code displays a table with sector numbers, the corresponding industry with June 2021 employee data without agriculture in the table. 
