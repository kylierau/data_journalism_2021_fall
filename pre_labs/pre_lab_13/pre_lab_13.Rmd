...
**Answer** This code outputs a data frame that shows all states, the whole state vote (Trump/Clinton) in the 2016 election as well as the percentage of voters for the winning candidate in each state. It also had the percentage of businesses that receieved PPP loans and the average amount of money provided by these PPP loans per employee. 
```{r}
...
**Answer** The plot is scattered with a positive trend line up the middle. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This plot has less of a positive trend and the data is much more scattered. 
```{r}
...
**Answer**This table shows the percentages of businesses receiving PPE as well as the average amount per employee. 
```{r}
reveal_data %>%
...
**Answer** This code outputs the correlation of the data, explaining the calculation. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This is similar to the output above with another r value. 
```{r}
...
**Answer** This table shows the mean ppp amount per employee for Clinton and Trump states. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** This code outputs a two sample t-test with the ppe amount per employee and the vote_2016 data. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** This code outputs another two sample t test using the percentages of businesses that received ppe with the vote_2016 data. 
```{r}
