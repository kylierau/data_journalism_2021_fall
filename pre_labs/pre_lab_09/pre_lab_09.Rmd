...
**Answer** This chart has 195865 rows and 63 columns. 
...
**Answer** The first line of code is reading in the ppp data, the second line is grouping by project county name, the third line is summarizing the data by total_loans and then it is arranged by total loans in a descending order. The "head(10)" piece of the code is making sure that the dataframe only shows 10 counties. 
...
**Answer** After running the code, I see a grey square, which is the empty ggplot container. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** After running this code, I see a bar chart of project county name on the x axis and total loan counts on the y axis (weight). This chart is in alphabetical order. 
...
**Answer** This code returns the same bar chart but reordered by total loans. This means that it is in ascending county order based on the amount of loans in each county. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** After running this code, there are style changes and the bars are coming out of the side of the chart instead of the bottom. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** After running the code, I see the background has changed from grey to white and the font has been changed. This is a more clean look than the chart above. 
...
**Answer** This code installed the package "ggthemes" and loaded it so that we could make styling changes to our gg plots.
...
**Answer** The last chart had an open white background with boxes, this chart has a plain blue background with white lines through each of the bars. It is very simple, yet  pleasing to the eye. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** There is now a title on this chart, as well as new axis titles "county" and "total loans". These make the chart more organized. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running the code, I see a data frame with columns called "date_approved" and "total_loans"
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running this code, I see a line chart that shows date_approved on the x axis and total_loans on the y axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** After running this code, there are now dates that show when the loans fall to zero. This chart is very messay but it has the weekly date breaks and more information than the last chart had. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** This chart has the same data and titles, but it is styled to be more well organized and readble with angled tick mark titles. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
