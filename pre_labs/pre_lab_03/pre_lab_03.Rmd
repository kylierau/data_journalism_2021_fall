...
**Answer** There were 87,927 rows in the output table showing errors. 
...
**Answer** When I run the codelock it does not show any parsing errors.
...
**Answer** The amount field is under the data type number. The name field is characters and the id column is a number. 
```{r}
...
**Answer**: There are 192959 loans when the franchise_name is NA. There are 275 loans when the franchise_name is Subway. 
...
**Answer**: There are also 135073 loans where the forgiveness_date is NA. 
...
**Answer**: There are 135073 loans where forgiveness_date and forgiveness_amount is NA. This is the same as the number for forgiveness_amount and forgiveness_date when they run separately. This means that we can conclude that these two categories correlate. 
```{r}
ppp_maryland_loans %>%
...
**Answer** There are no loans listed for October 2020. The data skips from August 2020 to January 2021. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10000000. The smallest loan is for 6 dollars. The gap in these amounts seems suspicous when only looking at these outliers. It also seams unrealistic that a business would receive a loan for 6 dollars. 
```{r}
...
**Answer** There are two loans under 100 dollars. 
```{r}
...
**Answer** The various fields with amounts within these two loans do align. 
...
**Answer** THe amount field is in characters. 
```{r}
...
**Answer** The amount column is stored as a character, not a number; there is a fully duplicated row (the first row is the same as the second); the zip field mixes 5 digit numbers with 11 digit numbers (this would not allow us to group and count the numbers of loans).  
...
**Answer** It changes the name of the column "1_id" to "x1_id". 
...
**Answer** The error says problem with 'summarise()` column `total_amount`.`total_amount = sum(amount)`. x invalid 'type' (character) of argument. I think this means that the total_amount argument is not a valid request. 
...
**Answer** The total amount is 203179
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** There is one duplicate and it is Stuart Jones LLC.
...
**Answer** Two mispellings of Arnold remain after running this code. 
...
**Answer** 1 mispelling of Arnold remains after running this code. 
...
**Answer** No mispellings of Arnold remain after running this code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
